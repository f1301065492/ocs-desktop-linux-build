...
软件已经把整条链路做进了设置页，**不需要你在命令行做任何事**：

打开「软件设置 → SSH 反向隧道」：

1. 点「生成密钥」——软件会生成一对专用 ed25519 密钥（放在应用数据目录，
   不会碰你已有的 `~/.ssh`），把显示出来的**公钥**复制走
...
- **掉线自动重连**（按退避重试，最长 30 秒一次）
- **认证失败这类不可自愈的错误不会空转重试**，而是停下并把要改什么直接写在错误区
- 软件退出时自动清理隧道进程

映射完成后，在 VPS 上用 `https://localhost:15320` 访问。

> ⚠️ **必须是 `localhost`，不能写 `127.0.0.1`**。
> 自签证书的 SAN 里只有 `localhost` 和各网卡的**局域网** IP，
> `127.0.0.1` 因为是回环地址被排除了。写成 `127.0.0.1` 会主机名校验失败，
> 而且**导入证书也救不回来，因为 SAN 里根本没有这个条目**。
